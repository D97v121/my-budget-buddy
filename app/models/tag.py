...
class Tags(db.Model):
    ...
    color = db.relationship('TagColor')  # <-- convenience
    transactions = db.relationship(
        'Transaction',
        secondary=transaction_tags,
        back_populates='tags'
    )
    __table_args__ = (db.UniqueConstraint('user_id', 'name', name='_user_tag_uc'),)
...
class TagColor(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('user_id','color_hex',name='_user_colorhex_uc'),)
